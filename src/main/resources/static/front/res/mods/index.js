                ...
                , picture: function (editor) { //插入图片
                    layer.open({
                        ...
                        , success: function (layero, index) {
                            ...
                            //执行上传实例。之后要在header放token
                            upload.render({
                                elem: '#uploadImg',   // 点击上传的按钮
                                url: '/upload/image',
                                field: 'image',
                                size: 1024 * 5,   // 5 MB。最大可允许上传的大小，单位 KB
                                accept: 'images',  // 上传前校验
                            ...
                            form.on('submit(uploadImages)', function (data) {
                                console.log(data);
                                // var field = data.field; // 表单中的字段
                                // if (!field.image) return image.focus();
                                // layui.focusInsert(editor[0], 'img[' + field.image + '] ');
                                if (!image.val) return image.focus();
                                layui.focusInsert(editor[0], 'img[' + image.val() + '] ');
                                layer.close(index);
                            });
                        }
                    });
